...
class BorrowReturnManagementScreen extends StatefulWidget {
  ...
  @override
  State<BorrowReturnManagementScreen> createState() =>
      _BorrowReturnManagementScreenState();
}

class _BorrowReturnManagementScreenState
    extends State<BorrowReturnManagementScreen> {
  bool isLoading = true;
  ...
  Future<void> _handleApproveBorrow(dynamic item) async {
    bool? isAssigned = await showDialog<bool>(
      context: context,
      barrierDismissible:
          false, // Bắt buộc thao tác, không cho bấm ra ngoài để tắt
      builder: (context) => AdminAssignMachineDialog(
        phieuId: item['id'],
        requiredQuantity:
            item['so_luong'] ??
            1, // Lấy số lượng GV yêu cầu, mặc định là 1 nếu null
      ),
    );
  ...
  Future<void> _confirmReturn(int returnId, int? borrowId) async {
    if (borrowId == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Phiếu trả này không liên kết với phiếu mượn nào!'),
          ...
          children: [
            ...
            Container(
              ...
              child: TabBar(
                ...
                tabs: const [
                  Tab(text: 'Danh Sách Phiếu Mượn'),
                  Tab(text: 'Lịch Sử Trả Máy'),
                ],
              ),
            ),
            ...
            Expanded(
              child: isLoading
                  ...
                  : TabBarView(
                      children: [_buildBorrowList(), _buildReturnList()],
                    ),
            ),
          ],
        ),
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        return _buildCardWrapper(
          ...
          status: status,
          title:
              'Mã phiếu: ${item['ma_phieu_muon']} - ${item['ten_nguoi_muon'] ?? 'Ẩn danh'}',
          ...
          actions:
              (status.toString().toLowerCase() == 'pending' ||
                  status.toString().toLowerCase() == 'chờ duyệt' ||
                  status.toString().toLowerCase() == 'borrowed' ||
                  status.toString().toLowerCase() == 'đang mượn')
              ? [
                  // NÚT TỪ CHỐI
                  OutlinedButton.icon(
                  ...
                  ElevatedButton.icon(
                    ...
                    onPressed: () =>
                        _handleApproveBorrow(item), // Đổi hàm thực thi ở đây
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        return _buildCardWrapper(
          ...
          status: 'Hoàn tất',
          title:
              'Mã phiếu trả: ${item['ma_phieu_tra']} - ${item['ten_nguoi_tra'] ?? 'Ẩn danh'}',
          ...
          actions: [
            ...
            ElevatedButton.icon(
              ...
              onPressed: () =>
                  _confirmReturn(item['id'], item['ma_phieu_muon']),
      ...
      child: ClipRRect(
        ...
        child: ExpansionTile(
          ...
          leading: CircleAvatar(
            backgroundColor:
                (type == 'muon' ? _getStatusColor(status) : Colors.blueGrey)
                    .withOpacity(0.15),
            radius: 24,
            child: Icon(
              type == 'muon' ? Icons.upload_rounded : Icons.download_rounded,
              color: type == 'muon' ? _getStatusColor(status) : Colors.blueGrey,
            ),
          ),
            ...
            child: Wrap(
              ...
              children: [
                Container(
                  ...
                  child: Text(
                    translateAppStatus(status, defaultLabel: 'Không rõ'),
                    style: TextStyle(
                      fontSize: 11,
                      color: type == 'muon'
                          ? _getStatusColor(status)
                        ...
                        children: [
                          ...
                          Expanded(
                            child: Column(
                              ...
                              children: [
                                Text(
                                  '${m['ten_may'] ?? 'N/A'} (Mã: ${m['ma_may'] ?? 'N/A'})',
                                  style: const TextStyle(
                                    fontWeight: FontWeight.bold,
                                  ),
                                ),
                                if (m['tinh_trang'] != null)
                                  Text(
                                    'Tình trạng: ${m['tinh_trang']}',
                                    style: TextStyle(
                                      color: Colors.grey.shade700,
                                      fontSize: 13,
                                    ),
                                  ),
                                if (m['ghi_chu_ct'] != null)
                                  Text(
                                    'Ghi chú: ${m['ghi_chu_ct']}',
                                    style: TextStyle(
                                      color: Colors.grey.shade700,
                                      fontSize: 13,
                                    ),
