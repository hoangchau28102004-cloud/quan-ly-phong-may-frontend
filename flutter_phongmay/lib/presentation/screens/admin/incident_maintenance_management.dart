...
class _IncidentMaintenanceManagementScreenState
    extends State<IncidentMaintenanceManagementScreen>
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      dynamic lRes;
      try {
        lRes = await ApiService.get('/nhat-ky-sua-chua');
      } catch (e) {
        debugPrint('Lỗi tải nhật ký: $e');
      }

      dynamic cRes, uRes;
      try {
        cRes = await ApiService.get('/may-tinh');
      } catch (e) {
        debugPrint('Lỗi tải máy: $e');
      }
      try {
        uRes = await ApiService.get('/nguoi-dung');
      } catch (e) {
      ...
      setState(() {
        ...
        logs = (lRes != null)
            ? (ApiService.decodeBody(lRes)?['data'] ?? [])
            : []; // Ép kiểu an toàn chống Null
        computers = (cRes != null)
            ? (ApiService.decodeBody(cRes)?['data'] ?? [])
            : [];
        users = (uRes != null)
            ? (ApiService.decodeBody(uRes)?['data'] ?? [])
            : [];
      });
    } catch (e) {
      if (mounted) {
  ...
  List<DropdownMenuItem<int>> _safeMap(
    List<dynamic> list,
    String emptyText,
  ...
  void _openTicketModal({
    Map<String, dynamic>? incidentItem,
    Map<String, dynamic>? ticketItem,
  }) {
    bool isFromApprove = incidentItem != null && ticketItem == null;

    int? selectedIncidentId = _getValidId(
      ticketItem?['ma_bao_cao_su_co'] ?? incidentItem?['id'],
      incidents,
    );
    int? selectedAssigneeId = _getValidId(
      ticketItem?['ma_nguoi_phu_trach'],
      users,
    );

    final cachXuLyCtrl = TextEditingController(
      text: ticketItem?['cach_xu_ly'] ?? '',
    );
    final chiPhiCtrl = TextEditingController(
      text: (ticketItem?['chi_phi'] ?? 0).toString(),
    );
    ...
    DateTime? startDate = ticketItem?['ngay_bat_dau'] != null
        ? DateTime.tryParse(ticketItem!['ngay_bat_dau'].toString())
        : DateTime.now();
    DateTime? endDate = ticketItem?['ngay_ket_thuc'] != null
        ? DateTime.tryParse(ticketItem!['ngay_ket_thuc'].toString())
        : null;
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    DropdownButtonFormField<int>(
                      isExpanded: true,
                      initialValue: selectedIncidentId,
                      ...
                      children: [
                          ...
                          child: DropdownButtonFormField<int>(
                            ...
                            decoration:
                                _customInputDecoration(
                                  ...
                                  fillColor: isFromApprove
                                      ? Colors.grey.shade100
                                      : Colors.white,
                                  filled: isFromApprove,
                                ),
                            items: _safeMap(
                              users,
                              'Chưa phân công',
                        ...
                        Expanded(
                          child: DropdownButtonFormField<String>(
                            isExpanded: true,
                            initialValue: loaiBaoTri,
                            decoration: _customInputDecoration('Loại bảo trì')
                                .copyWith(
                                  fillColor: isFromApprove
                                      ? Colors.grey.shade100
                                      : Colors.white,
                            ...
                            items: const [
                              ...
                              DropdownMenuItem(
                                value: 'Thay thế',
                                child: Text('Thay thế linh kiện'),
                              ),
                              DropdownMenuItem(
                                value: 'Vệ sinh',
                                child: Text('Vệ sinh/Bảo dưỡng'),
                              ),
                            ],
                            onChanged: isFromApprove
                                ? null
                                : (v) => setModalState(() => loaiBaoTri = v!),
                          ),
                        ),
                      ],
                    ),
                        ...
                        Expanded(
                          child: InkWell(
                            ...
                            child: Container(
                              ...
                              child: Text(
                                endDate != null
                                    ? 'Hoàn thành: ${DateFormat('dd/MM/yyyy').format(endDate!)}'
                                    : 'Ngày hoàn thành',
                                style: const TextStyle(fontSize: 14),
                              ),
                            ),
                    ...
                    Row(
                      children: [
                        Expanded(
                          child: TextField(
                            controller: chiPhiCtrl,
                            keyboardType: TextInputType.number,
                            readOnly: isFromApprove,
                            decoration: _customInputDecoration('Chi phí (VNĐ)')
                                .copyWith(
                                  fillColor: isFromApprove
                                      ? Colors.grey.shade100
                                      : Colors.white,
                        ...
                        Expanded(
                          child: DropdownButtonFormField<String>(
                            ...
                            decoration: _customInputDecoration('Trạng thái')
                                .copyWith(
                                  fillColor: isFromApprove
                                      ? Colors.grey.shade100
                                      : Colors.white,
                      ...
                      child: ElevatedButton(
                        ...
                        onPressed: () async {
                          if (selectedIncidentId == null) {
                            ScaffoldMessenger.of(context).showSnackBar(
                              const SnackBar(
                                content: Text('Vui lòng chọn Sự cố!'),
                                backgroundColor: Colors.red,
                              ),
                            );
                            return;
                          ...
                          final payload = {
                            'ma_bao_cao_su_co': selectedIncidentId,
                            'ma_nguoi_phu_trach': selectedAssigneeId,
                            ...
                            'ngay_ket_thuc': endDate != null
                                ...
                                : null,
                            'cach_xu_ly': cachXuLyCtrl.text,
                            'chi_phi': double.tryParse(chiPhiCtrl.text) ?? 0,
                          ...
                          try {
                            if (ticketItem == null) {
                              ...
                              messenger.showSnackBar(
                                const SnackBar(
                                  content: Text(
                                    'Duyệt sự cố & Tạo phiếu thành công!',
                                  ),
                              ...
                              await ApiService.put(
                                '/phieu-bao-tri/${ticketItem['id']}',
                                payload,
                              );

                              final originalIncident = incidents.firstWhere(
                                (inc) => inc['id'] == selectedIncidentId,
                                orElse: () => null,
                              );
                              if (originalIncident != null) {
                                String newIncidentStatus =
                                    originalIncident['trang_thai'];
                                ...
                                if (newIncidentStatus !=
                                    originalIncident['trang_thai']) {
                                  await ApiService.put(
                                    '/bao-cao-su-co/$selectedIncidentId',
                                    {
                                      'tieu_de': originalIncident['tieu_de'],
                                      'ma_may_tinh':
                                          originalIncident['ma_may_tinh'],
                                      'loai_su_co':
                                          originalIncident['loai_su_co'],
                              ...
                              messenger.showSnackBar(
                                const SnackBar(
                                  content: Text(
                                    'Cập nhật phiếu và đồng bộ sự cố thành công!',
                                  ),
                                  backgroundColor: Colors.green,
                                ),
                              );
                        ...
                        child: Text(
                          ticketItem == null ? 'DUYỆT & TẠO PHIẾU' : 'CẬP NHẬT',
                          style: const TextStyle(
                            color: Colors.white,
                            fontWeight: FontWeight.bold,
                            fontSize: 16,
  ...
  Future<void> _deleteItem(String endpoint, int id) async {
    final confirm = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: const Text('Bạn có chắc chắn muốn xóa mục này khỏi hệ thống?'),
        actions: [
          TextButton(
            ...
            child: DropdownButtonHideUnderline(
              child: DropdownButton<String>(
                ...
                items: const [
                  ...
                  DropdownMenuItem(
                    value: 'closed',
                    child: Text('Đã khắc phục'),
                  ),
                ],
                onChanged: (val) =>
                    setState(() => _incidentStatusFilter = val!),
              ),
            ),
          ),
            ...
            child: filteredIncidents.isEmpty
                ...
                : ListView.builder(
                    ...
                    itemBuilder: (ctx, i) {
                      ...
                      Color statusColor = inc['trang_thai'] == 'closed'
                          ? Colors.green
                          : (inc['trang_thai'] == 'in_progress'
                                ? Colors.orange
                                : Colors.red);
                            ...
                            children: [
                              ...
                              Row(
                                children: [
                                  ...
                                  Text(
                                    'Máy: ${inc['ten_may'] ?? 'N/A'} - ${inc['ten_phong'] ?? ''}',
                                    style: const TextStyle(
                                      fontWeight: FontWeight.w500,
                                    ),
                                  ),
                              ...
                              Row(
                                children: [
                                  ...
                                  const SizedBox(width: 8),
                                  Text(
                                    'Người báo: ${inc['nguoi_bao_cao'] ?? 'Ẩn danh'}',
                                  ),
                                ],
                              ),
                              const SizedBox(height: 6),
                              Row(
                                children: [
                                  ...
                                  Expanded(
                                    child: Text(
                                      'Mức độ: ${inc['muc_do']} | Loại: ${inc['loai_su_co']}',
                                      maxLines: 1,
                              ...
                              if (inc['trang_thai'] == 'open') ...[
                                ...
                                Row(
                                  ...
                                  children: [
                                    ElevatedButton.icon(
                                      onPressed: () =>
                                          _openTicketModal(incidentItem: inc),
                                      icon: const Icon(
                                        Icons.check_circle,
                                        size: 16,
                                      ),
            ...
            child: filteredTickets.isEmpty
                ...
                : ListView.builder(
                    ...
                    itemBuilder: (ctx, i) {
                      ...
                      Color statusColor = tic['trang_thai'] == 'completed'
                          ? Colors.green
                          : (tic['trang_thai'] == 'in_progress'
                                ? Colors.orange
                                : Colors.grey);
                      return Card(
                        margin: const EdgeInsets.only(bottom: 12),
                            ...
                            children: [
                              Row(
                                ...
                                children: [
                                  ...
                                  PopupMenuButton<String>(
                                    ...
                                    onSelected: (value) {
                                      if (value == 'edit') {
                                        _openTicketModal(ticketItem: tic);
                                      }
                                      if (value == 'delete') {
                                        _deleteItem(
                                          '/phieu-bao-tri',
                              ...
                              Row(
                                children: [
                                  ...
                                  const SizedBox(width: 8),
                                  Text(
                                    'Phụ trách: ${tic['nguoi_phu_trach'] ?? 'Chưa phân công'}',
                                  ),
                                ],
                              ),
                              const SizedBox(height: 6),
                              Builder(
                                builder: (context) {
                                  final double chiPhi =
                                      double.tryParse(
                                        tic['chi_phi']?.toString() ?? '0',
                                      ) ??
                                      0.0;
                                  final String formattedChiPhi =
                                      NumberFormat.currency(
                                        locale: 'vi',
                                        symbol: '',
                                      ).format(chiPhi).trim();
                                  return Text(
                                    'Chi phí: $formattedChiPhi VNĐ',
                                    style: const TextStyle(
                                      color: Colors.green,
                                      fontWeight: FontWeight.bold,
                                    ),
