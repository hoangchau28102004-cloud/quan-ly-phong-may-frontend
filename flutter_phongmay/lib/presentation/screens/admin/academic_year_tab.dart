  ...
  void _showFormBottomSheet([Map<String, dynamic>? yearData]) {
    showModalBottomSheet(
      ...
      builder: (ctx) => Padding(
        ...
        child: AddYearForm(initialData: yearData, onSuccess: _loadData),
      ),
    );
  }
  ...
  Future<void> _deleteYear(int id) async {
    final confirm = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: const Text(
          'Xóa năm học sẽ xóa luôn toàn bộ tuần học bên trong. Tiếp tục?',
        ),
        actions: [
          TextButton(
                            ...
                            children: [
                              ...
                              Row(
                                children: [
                                  ...
                                  Text(
                                    '${_formatDate(year['ngay_bat_dau'])} - ${_formatDate(year['ngay_ket_thuc'])}',
                                    style: TextStyle(
...
class AddYearForm extends StatefulWidget {
  ...
  const AddYearForm({super.key, this.initialData, required this.onSuccess});
      ...
      child: Form(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 8),
            Text(
              'Hệ thống sẽ tự động chia tuần học dựa trên khoảng thời gian bạn chọn.',
              style: TextStyle(
                color: Colors.grey.shade600,
                fontSize: 13,
            ...
            TextFormField(
              ...
              decoration: InputDecoration(
                labelText: 'Tên năm học (VD: 2024-2025)',
                    ...
                    try {
                      ...
                      if ((response.statusCode == 200 ||
                        ...
                        final errorMsg =
                            ...
                            'Lỗi hệ thống hoặc Database Backend';
                        if (context.mounted) {
                          ScaffoldMessenger.of(context).showSnackBar(
                            SnackBar(
                              content: Text('Thất bại: $errorMsg'),
                              backgroundColor: Colors.red,
                            ),
                          );
                        }
                ...
                : ListView.builder(
                    ...
                    itemBuilder: (ctx, i) {
                      ...
                      final start = DateFormat(
                        'dd/MM/yyyy',
                      ).format(DateTime.parse(w['ngay_bat_dau']));
                      final end = DateFormat(
                        'dd/MM/yyyy',
                      ).format(DateTime.parse(w['ngay_ket_thuc']));
                      return ListTile(
                        leading: CircleAvatar(
                          backgroundColor: const Color(
                            0xFF193D87,
                          ).withOpacity(0.1),
                          child: Text('${w['so_tuan']}'),
                        ),
                        title: Text(
                          'Tuần ${w['so_tuan']}',
