...
class CourseSectionDetailScreen extends StatefulWidget {
  ...
  @override
  State<CourseSectionDetailScreen> createState() => _CourseSectionDetailScreenState();
}
...
class _CourseSectionDetailScreenState extends State<CourseSectionDetailScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      context.read<AcademicViewModel>().fetchStudentsForModule(widget.sectionItem.id);
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text('Sinh viên LHP: ${widget.sectionItem.maLopHocPhan}', style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 18)),
          ...
          Container(
            ...
            child: Column(
              children: [
                Row(
                  children: [
                    Expanded(child: _buildInfoBox('MÃ LHP', widget.sectionItem.maLopHocPhan)),
                    const SizedBox(width: 8),
                    Expanded(child: _buildInfoBox('MÔN HỌC', widget.sectionItem.tenMon ?? 'N/A')),
                  ],
                ),
                const SizedBox(height: 8),
                Row(
                  children: [
                    Expanded(child: _buildInfoBox('SĨ SỐ', '${viewModel.moduleStudents.length} / ${widget.sectionItem.siSoToiDa}')),
                    const SizedBox(width: 8),
                    Expanded(child: _buildInfoBox('GIẢNG VIÊN', widget.sectionItem.tenGiangVien ?? 'Chưa phân công')),
                  ],
                ),
              ],
            ),
          ...
          Expanded(
            child: viewModel.isStudentLoading 
              ...
              : displayedStudents.isEmpty
                ? const Center(child: Text('Không tìm thấy sinh viên nào.', style: TextStyle(color: Colors.grey)))
                : ListView.builder(
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Text(value, style: const TextStyle(fontSize: 14, fontWeight: FontWeight.bold, color: Color(0xFF0F3E99)), maxLines: 1, overflow: TextOverflow.ellipsis),
        ],
      ),
    );
  }
  ...
  Widget _buildStudentCard(BuildContext context, AcademicViewModel viewModel, StudentModel sv) {
    return Card(
      ...
      child: ListTile(
        ...
        title: Text(sv.hoTen, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 15)),
        subtitle: Padding(
          padding: const EdgeInsets.only(top: 4.0),
          child: Text('${sv.maSinhVien} • Khóa: ${sv.nienKhoa ?? "N/A"}\n${sv.email ?? "Chưa có email"}', style: const TextStyle(fontSize: 12, height: 1.4)),
        ),
        trailing: IconButton(
        ...
        builder: (context, setState) {
          return Container(
            ...
            child: Column(
              ...
              children: [
                const Text('Thêm Sinh Viên vào LHP', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold, color: Color(0xFF0F3E99)), textAlign: TextAlign.center),
                ...
                DropdownButtonFormField<int>(
                  ...
                  items: viewModel.allStudents.map((sv) => DropdownMenuItem(value: sv.id, child: Text('${sv.maSinhVien} - ${sv.hoTen}'))).toList(),
                  onChanged: (val) => setState(() => selectedStudentId = val),
                  hint: viewModel.allStudents.isEmpty ? const Text('Không có sinh viên nào') : const Text('Chọn sinh viên cần thêm'),
                ),
                const SizedBox(height: 24),
                ElevatedButton(
                  ...
                  onPressed: selectedStudentId == null ? null : () async {
                    if (viewModel.moduleStudents.length >= widget.sectionItem.siSoToiDa) {
                      if (!ctx.mounted) return;
                      ScaffoldMessenger.of(context).showSnackBar(
                        const SnackBar(content: Text('Không thể thêm: Lớp học phần đã đạt sĩ số tối đa!'), backgroundColor: Colors.redAccent)
                      );
                      return; 
                    }
                    ...
                    final success = await viewModel.addStudentToModule(widget.sectionItem.id, selectedStudentId!);
                    if (!ctx.mounted) return;
                    ...
                    if (success) {
                      ...
                      messenger.showSnackBar(const SnackBar(content: Text('Đã thêm sinh viên!'), backgroundColor: Colors.green));
                    } else {
                      messenger.showSnackBar(const SnackBar(content: Text('Lỗi: Sinh viên đã tồn tại hoặc có lỗi xảy ra!'), backgroundColor: Colors.redAccent));
                    }
                  },
                  child: const Text('THÊM VÀO LỚP HỌC PHẦN', style: TextStyle(fontSize: 16, color: Colors.white, fontWeight: FontWeight.bold)),
                ),
                const SizedBox(height: 24),
              ],
            ),
  ...
  void _showRemoveConfirm(BuildContext context, AcademicViewModel viewModel, StudentModel sv) {
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        title: const Text('Xóa khỏi lớp học phần', style: TextStyle(color: Colors.red, fontWeight: FontWeight.bold)),
        content: Text('Bạn có muốn xóa sinh viên ${sv.hoTen} ra khỏi Lớp học phần này không?'),
        ...
        actions: [
          ...
          ElevatedButton(
            ...
            onPressed: () async {
              ...
              final success = await viewModel.removeStudentFromModule(widget.sectionItem.id, sv.id);
              if (!ctx.mounted) return;
              ...
              if (success) {
                // GỌI MESSENGER ĐÃ LƯU
                messenger.showSnackBar(const SnackBar(content: Text('Đã xóa sinh viên khỏi LHP'), backgroundColor: Colors.green));
              }
            },
